...
static bool reset_on_real_trigger = false;
...
static Stage initial_gaster_stage = STAGE_RESET;
static const char*
stage_to_string(Stage stage) {
    switch (stage) {
        case STAGE_RESET:
            return "STAGE_RESET";
        case STAGE_SETUP:
...
gaster_checkm8(usb_handle_t *handle) {
    Stage stage = initial_gaster_stage;
    bool ret = 0, pwned = false;
    ...
    while (stage != STAGE_PWNED && wait_usb_handle(handle, checkm8_check_usb_device, &pwned)) {
        if(!pwned) {
            if(stage == STAGE_RESET) {
                ...
                // attempt at an hack for A7 SoC that is opt-in
...
args_config(int argc, char **argv) {
    ...
    for (int i = 1; i < argc && !stop; i++) {
        ...
        if (arg[0] == '-') {
            ...
            } // does not run the spray/whatever phase. if this fails (very likely), i will port the newer gaster fork with A7 fixes
            else if ((dashes > 1) ? 0 == strcmp(arg, "stage") : 0 == strcmp(arg, "s")) {
                if (initial_gaster_stage > 0) {
                    invalid = E_DUPLICATE;
                } else {
                    arg_target = (int*)&initial_gaster_stage;
                }
    ...
    if (help || invalid != E_NONE) {
        LOG_INFO(
            ...
            "  --stage, -s <number> = start from stage 'n' for pwning. stages:\n"
            "     STAGE_RESET=0, STAGE_SETUP=1, STAGE_SPRAY=2, STAGE_PATCH=3, STAGE_PWNED=4\n"
...
int main(int argc, char **argv) {
    ...
    const int wait = 3000;
    LOG_INFO("Waiting %d seconds before booting pongo", wait);
    sleep_ms(wait);
